...
// Parse the model's JSON output robustly: direct parse, then strip markdown
...

{
  ...
  "services": [
    { "title": "Service feature 1", "description": "Description of this feature", "icon": "Shield", "image": "PICK_RELEVANT_URL_FROM_POOL" },
    { "title": "Service feature 2", "description": "Description of this feature", "icon": "Award", "image": "PICK_RELEVANT_URL_FROM_POOL" },
    { "title": "Service feature 3", "description": "Description of this feature", "icon": "Clock", "image": "PICK_RELEVANT_URL_FROM_POOL" },
    { "title": "Service feature 4", "description": "Description of this feature", "icon": "CheckCircle", "image": "PICK_RELEVANT_URL_FROM_POOL" }
  ],
  "benefits": [
    "Benefit 1",
    "Benefit 2",
...
function buildStubPageData(serviceName: string) {
  return {
    hero: {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const stubPageData = buildStubPageData(serviceName);

    // Real AI generation when an API key is configured; stub otherwise.
    if (process.env.ANTHROPIC_API_KEY) {
      try {
        ...
        const message = await anthropic.messages.create({
          ...
          max_tokens: 8192,
          messages: [{ role: 'user', content: buildPrompt(serviceName, template) }],
        });
        const responseText = message.content
          .filter((block) => block.type === 'text')
          .map((block) => (block as { type: 'text'; text: string }).text)
          .join('');
        const pageData = parseModelJson(responseText);
        return NextResponse.json(pageData);
      } catch (aiError: unknown) {
        console.error('AI page generation failed, falling back to stub:', aiError);
        const note =
          aiError instanceof Error ? aiError.message : 'AI generation failed';
        return NextResponse.json({ ...stubPageData, aiError: note });
      }
    }
