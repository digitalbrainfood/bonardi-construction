...
function buildStubBlogData(topic: string, keywords: string[]) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const stubBlogData = buildStubBlogData(topic, keywords);
    ...
    if (process.env.ANTHROPIC_API_KEY) {
      try {
        ...
        const message = await anthropic.messages.create({
          ...
          max_tokens: 8192,
          messages: [{ role: 'user', content: buildPrompt(topic, keywords) }],
        });
        const responseText = message.content
          .filter((block) => block.type === 'text')
          .map((block) => (block as { type: 'text'; text: string }).text)
          .join('');
        const blogData = parseModelJson(responseText);
        return NextResponse.json(blogData);
      } catch (aiError: unknown) {
        console.error('AI blog generation failed, falling back to stub:', aiError);
        const note =
          aiError instanceof Error ? aiError.message : 'AI generation failed';
        return NextResponse.json({ ...stubBlogData, aiError: note });
      }
    }
